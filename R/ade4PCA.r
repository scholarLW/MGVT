...
ade4PCA <- function(file, groupFile=NULL, matrix.transpose=TRUE, center = TRUE, nf = 2, z=FALSE){
  if(is.null(file)){
  ...
  dudi.pca <- dudi.pca(tmp, center = center, scale = FALSE, scannf = FALSE, nf = nf)  
  kk = 2
  if(z){
    kk = 3
  }
  ...
  # pc1 and pc2
  if(ncol(da) > n+1){
    points = as.data.frame(cbind(SampleID,da[,(n+1):ncol(da)],dudi.pca$li[,1:kk]))
  }else{
    points = as.data.frame(cbind(SampleID,HostID,dudi.pca$li[,1:kk]))
  }
  if(z){
  ...
  eig <- 100 * dudi.pca$eig/sum(dudi.pca$eig)  
  cons = round(eig[1:kk],2) # contribution
